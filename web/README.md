...
Standalone website version of the Jupyter virtual keyboard with **model-backed autocomplete** (and browser fallback). Input Traditional Mongolian Unicode using the character set from `assets/token/new_char_to_token.json`, rendered with the project font.
...
2. **Run the app** (recommended)  
   ...
   The Suggest feature uses the server model (`/api/suggest`) when available, with a local browser fallback if the API is unavailable.
...
- **Suggest** – Get autocomplete suggestions from the custom model/tokenizer API, with local browser fallback if the API is unavailable. Pick one to insert that completion plus a space.
...
- `app.js` – Keyboard, model-backed suggestions with local fallback, suggestion buttons, sync, copy.
